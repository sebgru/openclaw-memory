...
class ApiTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.http = ThreadingHTTPServer(("127.0.0.1", 0), server.Handler); Thread(target=cls.http.serve_forever, daemon=True).start()
    @classmethod
    def tearDownClass(cls): cls.http.shutdown()
    def request(self, path):
        c = HTTPConnection(*self.http.server_address); c.request("GET", path); r = c.getresponse(); return r.status
