    ...
    for number, line in enumerate(markdown.splitlines(), 1):
        ...
        if match:
            if any(x.strip() for x in lines):
                sections.append((heading, start, lines))
    ...
    for title, start_line, body in sections:
        ...
        for paragraph in paragraphs:
            ...
            if current and len(candidate) > max_chars:
                chunks.append(MarkdownChunk(title, current, current_line))
                current, current_line = paragraph, start_line
            elif len(candidate) <= max_chars:
                current = candidate
            else:
                for offset in range(0, len(paragraph), max_chars):
                    part = paragraph[offset : offset + max_chars]
                    chunks.append(MarkdownChunk(title, part, start_line + offset // 80))
                current = ""
        if current:
            chunks.append(MarkdownChunk(title, current, current_line))
    return chunks
