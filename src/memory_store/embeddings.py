...
def hash_embedding(text: str, dimensions: int = 128) -> list[float]:
    ...
    if dimensions < 8: raise ValueError("dimensions must be at least 8")
...
class EmbeddingClient:
    ...
    def embed(self, text):
        if not self.url: return hash_embedding(text, self.dimensions)
        request = Request(self.url, data=json.dumps({"model": self.model, "input": [text]}).encode(), headers={"Content-Type": "application/json"})
        ...
        vector = (payload.get("data", [{}])[0].get("embedding")
                  if payload.get("data") else None)
        if vector is None:
            vector = payload["embeddings"][0]
        if len(vector) != self.dimensions: raise ValueError("embedding dimensions do not match configuration")
        return vector
